...
qsave(ad_ks_res, file = file.path(dir_output, "pgx_rna_before_alignment_ad_ks.qs"))
...
qsave(ad_ks_res, file = file.path(dir_output, "pgx_rna_after_alignment_ad_ks.qs"))
...
ad_ks_res_raw <- qread(file.path(dir_output, "pgx_rna_before_alignment_ad_ks.qs"))
ad_ks_res_raw <- data.frame(group = "uncorrected", ad_ks_res_raw)

ad_ks_res_raw <- data.frame(method = "uncorrected",
                     group = "uncorrected",
                     perc = table(ad_ks_res_raw$ad_padj_BH < 0.05)["TRUE"]/nrow(ad_ks_res_raw) *100)

ad_ks_res_corrected <- qread(file.path(dir_output, "pgx_rna_after_alignment_ad_ks.qs"))
...
pdf(file= file.path(dir_output, "ad_test_celligner.pdf"), width = 5, height = 4)
...
cell_line_tumor_distance_distribution(alignment, cor_res, "correlation_aligned", dir_output)

# plot raw (or uncorrected) data

cor_res <- calc_uncorrected_tumor_CL_correlation(TCGA_mat, pset_mat)
plot_uncorrected_distribution_of_CL_tumor_distances(cor_res, alignment, "correlation_raw", dir_output)
...
ggsave(paste(paste(dir_output, "correlation_aligned_raw", sep="") ,".pdf" , sep=""), 
       width = 3, height = 2.5)
