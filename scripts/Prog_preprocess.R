...
dir_input <- '/home/bioinf/bhklab/farnoosh/SARC/Data'
dir_output <- '/home/bioinf/bhklab/farnoosh/STS-PGx-Biomarker/data'
...
####################################
## microarray and drug response data
####################################

pset_name <- c("CCLE", "GDSCv1", "GDSCv2", "NCI")

# generate microarray (rna) data
...
pset_name <- c("CCLE", "CTRP", "GDSCv1", "GDSCv2", "NCI")
...
# update NCI AAC data 
aac_nci <- qread("/home/bioinf/bhklab/farnoosh/SARC/PGx_sts/Data/NCI_aac_data.qs")
drug_data$NCI$cell_ann_seq <- aac_nci$annot_cl
drug_data$NCI$drug_aac <- aac_nci$aac

dat <- list(rna = rna_data, aac = drug_data)
qsave(dat, file= file.path(dir_output, "PGx_sarc_data.qs"))
...
dat <- qread(file= file.path(dir_input, "GSE21122_RangedSummarizedExperiment_2022-06-11.qs"))
...
qsave(dat, file=  file.path(dir_output, "GSE21122_sarc_data.qs"))
# 149 samples, 11,245 genes 

### GSE21050 (microarray, STS)

dat <- qread(file= file.path(dir_input, "GSE21050_SummarizedExperiment_2022-06-11.qs"))
exp_mat <- assay(dat)
exp_mat <- t(exp_mat)
...
qsave(dat, file=  file.path(dir_output, "GSE21050_sarc_data.qs"))
# 282 samples, 16,014 genes 

### GSE30929 (microarray, STS)

dat <- qread(file= file.path(dir_input, "GSE30929_RangedSummarizedExperiment_2022-06-21.qs"))
exp_mat <- assay(dat)
exp_mat <- t(exp_mat)
...
qsave(dat, file=  file.path(dir_output, "GSE30929_sarc_data.qs"))
...
######################################################################################################
# GSE21122
dat <- qread(file = file.path(dir_output, "GSE21122_sarc_data.qs"))
...
# GSE21050
dat <- qread(file = file.path(dir_output, "GSE21050_sarc_data.qs"))
dat_ann <- dat$cell_ann_marray
...
# GSE30929
dat <- qread(file = file.path(dir_output, "GSE30929_sarc_data.qs"))
dat_ann <- dat$cell_ann_marray
...
dat <- qread(file.path(dir_output, "PGx_sarc_data.qs"))
rna_data <- dat$rna
...
nci_ann <- rna_data$NCI$cell_ann_marray
nci_ann$sampleid <- paste(nci_ann$sampleid, "nci", sep ="-")
nci_mat <- rna_data$NCI$marray_exp
...
pset_ann <- data.frame(sampleID = c(ccle_ann$sampleid,
                       ...
                       Type = c(rep("CCLE", length(ccle_ann$sampleid)),
                                ...
                                rep("NCI", length(nci_ann$sampleid))) )

int <- intersect(intersect(colnames(gdsc_mat),
                           intersect(colnames(ccle_mat), colnames(nci_mat))), colnames(tcga_mat)) # 11049
...
qsave(dat, file= file.path(dir_output, "PGx_gse_rna.qs"))

##################################################################
##  separate STS and Bone for alignment: Microarray 
##################################################################

dat <- qread(file.path(dir_output, "PGx_gse_rna.qs"))
